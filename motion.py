...
DIAGONAL = distance((0, 0), (WIDTH, HEIGHT))
...
GAUSSIAN_BLUR_FACTOR_2 = 3 #Tamano de la matriz usada para eliminar ruido la segunda vez
BINARY_THRESHOLD_1 = 5 #Umbral de binarizacion inicial
BINARY_THRESHOLD_2 = 240 #Umbral de binarizacion final
DILATION_FACTOR = 0 #Tamano de la matriz usada para dilatar las manchas de movimiento
...
motion1 = cv.CreateImage( cv.GetSize(frame), cv.IPL_DEPTH_8U, 1 ) #copia usada para binarizacion y grescale
motion2 = cv.CreateImage( cv.GetSize(frame), cv.IPL_DEPTH_8U, 1 ) #copia usada para binarizacion y grescale
...
while True:
  ...
  #frame1_features = cv.FindContours( frame1_1C, mem_storage, cv.CV_RETR_CCOMP, cv.CV_CHAIN_APPROX_SIMPLE )
